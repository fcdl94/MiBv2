...
class RandomResizeLong:
    ...
    def __call__(self, img, lbl=None):
        ...
        img = F.resize(img, target_shape, InterpolationMode.BILINEAR)
        if lbl is not None:
            if isinstance(lbl, torch.Tensor):
                lbl = F.resize(lbl, target_shape, InterpolationMode.BILINEAR)
            else:
                lbl = F.resize(lbl, target_shape, InterpolationMode.NEAREST)
            return img, lbl
        return img